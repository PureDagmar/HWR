...
from ..utils import file_to_img, process_result, process_image
...
model_labels = {v: k for k, v in model.labels.items()}
...
async def inference(blank: UploadFile):
    ...
    for key in cell_images:
        ...
        for row in cell_images[key]:
            images = torch.stack(
                [transform(process_image(img)) for img in row]
            ).to(sc.device)
            labels = model(images).detach().cpu().numpy()
            labels_row.append([
                process_result(label, model_labels) for label in labels
            ])
